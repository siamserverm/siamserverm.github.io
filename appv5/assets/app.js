...
function idbPut(id,blob){return new Promise((res,rej)=>{let tx=db.transaction(IMG_STORE,'readwrite');tx.objectStore(IMG_STORE).put(blob,id);tx.oncomplete=res;tx.onerror=()=>rej(tx.error)})}
function idbGet(id){return new Promise((res,rej)=>{let tx=db.transaction(IMG_STORE,'readonly');let r=tx.objectStore(IMG_STORE).get(id);r.onsuccess=()=>res(r.result||null);r.onerror=()=>rej(r.error)})}
function idbDel(id){return new Promise((res,rej)=>{let tx=db.transaction(IMG_STORE,'readwrite');tx.objectStore(IMG_STORE).delete(id);tx.oncomplete=res;tx.onerror=()=>rej(tx.error)})}
...
async function init(){document.documentElement.dataset.theme=localStorage.getItem('alm_theme')||'dark';db=await openDb();products=loadProducts();await migrateOldImages();bind();loadSettings();updateWorkspaceBadge();await render();updatePreview();updateSlugStatus()}init().catch(e=>alert('เปิดระบบจัดเก็บรูปไม่สำเร็จ: '+e.message));
